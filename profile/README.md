...
**Create-Flex**는 크리에이터 매니지먼트 전문 인트라넷 플랫폼입니다.  
크리에이터와 담당 직원(매니저/관리자)이 하나의 통합 시스템에서 효율적으로 협업할 수 있도록 설계된 **B2B 사내 크리에이터 관리 솔루션**입니다.

기존에 분산되어 있던 크리에이터 관리 업무(일정, 휴가, 업무 배분 등)를 하나의 플랫폼으로 통합하여 **업무 효율성 향상**과 **체계적인 크리에이터 생애주기 관리**를 목표로 합니다.
...
| 역할 | 설명 |
|------|------|
| `ADMINISTRATOR` | 시스템 전체 관리, 계정 및 권한 관리 |
| `MANAGER` | 크리에이터 담당 매니저, 업무 배분 및 일정 관리 |
| `EMPLOYEE` | 일반 직원, 업무 확인 및 협업 |
| `CREATOR` | 크리에이터 본인, 일정 및 활동 내역 확인 |

## 핵심 가치

- **역할별 맞춤 뷰**: 사용자 역할에 따라 최적화된 UI/UX 제공
- **실시간 협업**: 팀 간 원활한 커뮤니케이션 및 업무 공유
- **통합 관리**: 크리에이터 생애주기 전반을 하나의 시스템에서 관리
- **보안**: JWT 기반 인증으로 역할별 접근 권한 제어
